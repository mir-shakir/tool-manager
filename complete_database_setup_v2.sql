-- ------------------------------------------------------------------------------------------------
-- ToolShelf MVP: Complete Database Setup Script (v2 - Corrected)
-- ------------------------------------------------------------------------------------------------
-- Instructions:
-- 1. If you ran the previous script, please delete all the created tables first to start fresh.
...
-- SECURITY DEFINER allows this function to bypass RLS policies, which is crucial for breaking recursion.
...
-- ** THIS IS THE FIX **
-- The SELECT policy now uses the SECURITY DEFINER function to check for team membership, avoiding recursion.
CREATE POLICY "Users can view members of teams they belong to." ON public.team_members FOR SELECT USING (get_my_role_in_team(team_id) IS NOT NULL);
